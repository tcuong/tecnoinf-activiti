...
public class DefaultViewManager implements ViewManager {
	...
	public void showTaskPage(String taskId) {
		...
		if (task == null) {
			...
			Iterator<IdentityLink> identityLinkIterator = identityLinks
					.iterator();
			...
			while (!pageFound && identityLinkIterator.hasNext()) {
				...
				if (identityLink.getGroupId() != null
						&& groupIds.contains(identityLink.getGroupId())) {
		...
		ExplorerApp
				...
				.showErrorNotification(
						...
						ExplorerApp
								...
								.getI18nManager()
								.getMessage(
										Messages.NAVIGATION_ERROR_NOT_INVOLVED,
										taskId));
	}
	...
	public void showTasksPage(String taskId) {
		switchView(new TasksPage(taskId), ViewManager.MAIN_NAVIGATION_TASK,
				TaskMenuBar.ENTRY_TASKS);
	}
	...
	public void showQueuedPage(String groupId, String taskId) {
		switchView(new QueuedPage(groupId, taskId),
				ViewManager.MAIN_NAVIGATION_TASK, TaskMenuBar.ENTRY_QUEUED);
	}
	...
	public void showInvolvedPage(String taskId) {
		switchView(new InvolvedPage(taskId), ViewManager.MAIN_NAVIGATION_TASK,
				TaskMenuBar.ENTRY_INVOLVED);
	}
	...
	public void showArchivedPage(String taskId) {
		switchView(new ArchivedPage(taskId), ViewManager.MAIN_NAVIGATION_TASK,
				TaskMenuBar.ENTRY_ARCHIVED);
	}
	...
	public void showProcessDefinitionPage() {
		switchView(new ProcessDefinitionPage(),
				...
				ProcessMenuBar.PROCESS_DEFINITIONS);
	}
	...
	public void showDatabasePage() {
		switchView(new DatabasePage(), ViewManager.MAIN_NAVIGATION_MANAGE,
				ManagementMenuBar.ENTRY_DATABASE);
	}

	public void showDatabasePage(String tableName) {
		switchView(new DatabasePage(tableName),
	...
	public void showJobPage() {
		switchView(new JobPage(), ViewManager.MAIN_NAVIGATION_MANAGE,
				ManagementMenuBar.ENTRY_JOBS);
	}

	public void showJobPage(String jobId) {
		switchView(new JobPage(jobId), ViewManager.MAIN_NAVIGATION_MANAGE,
				ManagementMenuBar.ENTRY_JOBS);
	}

	public void showUserPage() {
		switchView(new UserPage(), ViewManager.MAIN_NAVIGATION_MANAGE,
				ManagementMenuBar.ENTRY_USERS);
	}

	public void showUserPage(String userId) {
		switchView(new UserPage(userId), ViewManager.MAIN_NAVIGATION_MANAGE,
				ManagementMenuBar.ENTRY_USERS);
	}

	public void showGroupPage() {
		switchView(new GroupPage(), ViewManager.MAIN_NAVIGATION_MANAGE,
				ManagementMenuBar.ENTRY_GROUPS);
	}

	public void showGroupPage(String groupId) {
		switchView(new GroupPage(groupId), ViewManager.MAIN_NAVIGATION_MANAGE,
				ManagementMenuBar.ENTRY_GROUPS);
	}
	...
	protected void switchView(AbstractTablePage page, String mainMenuActive,
			String subMenuActive) {
		currentPage = page;
		mainWindow.setMainNavigation(mainMenuActive);
		mainWindow.switchView(page);
		page.getToolBar().setActiveEntry(subMenuActive); // Must be set AFTER
															// adding page to
															// window (toolbar
															// will be created
															// in atach())
